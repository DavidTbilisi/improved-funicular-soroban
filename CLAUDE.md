...
- **`src/domain/`** — pure logic, no DOM, no globals.
  - `pegs.js` = frozen data tables only (A–Z food pegs, cube faces, audio/visual matrix pegs, hex A–F). `faces.js` = digit→face grammar. `config.js` = abacus geometry (`INT_COLS=11`, `FRAC_COLS=4`, place names). `rod.js` = the numeric core (`{sky,earth}` rods; `index 0 = ones/tenths`). `number.js` = parse/display/`decodeChips`.
...
- **`src/view/`** — one class per panel (`soroban`, `readout`, `deepPack`, `reference`, `drill`). Views **observe** the store/session and emit user intent through injected callbacks; they never mutate state directly.
